...
Real calcCableSpanLength(const State& s, const std::vector<CableSpan>& cables) {
    SimTK::Real length = 0.;
    for (const SimTK::CableSpan& cable : cables) {
        length += cable.calcLength(s);
    }
    return length;
}

double calcCableSpanLengtheningSpeed(
        const State& s, const std::vector<CableSpan>& cables) {
...
void addCableSpanInEquivalentForces(const State& s,
        const std::vector<CableSpan>& cables, const double& tension,
        SimTK::Vector_<SimTK::SpatialVec>& bodyForces) {

    /* const SimTK::Vector_<SimTK::SpatialVec> before = bodyForces; */

    for (const SimTK::CableSpan& cable : cables) {
        cable.applyBodyForces(s, tension, bodyForces);
    }

    /* std::cout << "Print addCableSpanInEquivalentForces\n"; */
    /* for (int i = 0; i < bodyForces.size(); ++i) { */
    /*     SimTK::SpatialVec diff = bodyForces[i] - before[i]; */
    /*     if (diff.norm() > 1e-13) { */
    /*         std::cout << "Applied force at " << i << " = " << diff << std::endl; */
    /*     } */
    /* } */
}
...
void GeometryPath::printCableSpan(const SimTK::State& s) const {
    if (!m_useCableSpan) { return; }

    auto getLocationInGround = [&](MobilizedBodyIndex body,
                                       SimTK::Vec3 station) -> SimTK::Vec3 {
        return _model->getMatterSubsystem()
                .getMobilizedBody(body)
                .getBodyTransform(s)
                .shiftFrameStationToBase(station);
    };

    std::cout << "Print cable\n";
    for (const SimTK::CableSpan& cable : m_cableSpans) {
        const SimTK::Vec3 p0 = getLocationInGround(
                cable.getOriginBodyIndex(), cable.getOriginStation());
        const SimTK::Vec3 p1 = getLocationInGround(
                cable.getTerminationBodyIndex(), cable.getTerminationStation());
        std::cout << "p0 = " << p0 << ", p1 = " << p1
                  << ", lApprox = " << (p1 - p0).norm()
                  << ", l = " << cable.calcLength(s) << std::endl;
    }
}

// this is what is stored in a cache variable at runtime. It's a value-type
// that's safe to pass between multiple models, which looks up a point in the
// component path
    ...
    static PathElementLookup fromPtr(const OpenSim::PathPointSet& pps,
        ...
        if (owner == &geomPath) {
            ...
            for (int i = 0, len = pws.getSize(); i < len; ++i) {
                if (&pws[i] == owner) {
                    PointType t = ptr == &pws[i].getWrapPoint1()
                                          ? PointType::PathWrapPt1
                                          : PointType::PathWrapPt2;
        ...
        OPENSIM_THROW(OpenSim::Exception,
                "unable to create a PathElementLookup for the given pointer: "
                "are you sure it is part of the GeometryPath?");
    }
...
private:
    ...
    PathElementLookup(PointType pointType, int index)
            : index_{index}, pointType_{pointType} {}

public:
    OpenSim::AbstractPathPoint* toPtr(const OpenSim::PathPointSet& pps,
            const OpenSim::PathWrapSet& pws) const {
        switch (pointType_) {
        case PointType::InPathPointSet: return &pps.get(index_);
        case PointType::PathWrapPt1:
            return const_cast<OpenSim::PathWrapPoint*>(
                    &pws.get(index_).getWrapPoint1());
        case PointType::PathWrapPt2:
            return const_cast<OpenSim::PathWrapPoint*>(
                    &pws.get(index_).getWrapPoint2());
        default:
            OPENSIM_THROW(OpenSim::Exception,
                    "unhandled PointType passed to "
                    "`toPtr`: this is a developer error");
        }
    }
...
static void PopulatePathPointersCache(const OpenSim::PathPointSet& pps,
        const OpenSim::PathWrapSet& pws,
...
static void PopulatePathElementLookup(const OpenSim::PathPointSet& pps,
        const OpenSim::PathWrapSet& pws,
    ...
    for (int i = 0, len = ptrs.getSize(); i < len; ++i) {
        out.push_back(OpenSim::GeometryPath::PathElementLookup::fromPtr(
                pps, pws, geomPath, ptrs[i]));
    }
}
...
void GeometryPath::extendConnectToModel(Model& aModel) {
    ...
    OPENSIM_THROW_IF_FRMOBJ(get_PathPointSet().getSize() < 2,
            InvalidPropertyValue, getProperty_PathPointSet().getName(),
            "A valid path must be connected to a model by at least two "
            "PathPoints.")
...
void GeometryPath::extendAddToSystem(SimTK::MultibodySystem& system) const {
    ...
    if (m_useCableSpan) {
        extendAddCableSpanToSystem(system);
        return;
    }
    ...
    this->_currentPathCV = addCacheVariable("current_path",
            std::vector<PathElementLookup>{}, SimTK::Stage::Position);
    ...
    this->_colorCV = addCacheVariable(
            "color", get_Appearance().get_color(), SimTK::Stage::Topology);
}

void GeometryPath::extendAddCableSpanToSystem(
        SimTK::MultibodySystem& system) const {
    if (m_data.segments.size() == 0) {
        throw std::runtime_error("Must have at least one segment data");
    }

    auto findBody =
            [&](const std::string& frameAbsPath) -> SimTK::MobilizedBodyIndex {
        const PhysicalFrame* frame =
                _model->findComponent<PhysicalFrame>(frameAbsPath);
        SimTK_ASSERT1_ALWAYS(
                frame, "Could not find frame %s in model", frameAbsPath);
        return frame->getMobilizedBodyIndex();
    };

    SimTK::CableSubsystem& cableSubsystem = _model->updCableSubsystem();

    std::vector<SimTK::CableSpan>& cableSpans =
            const_cast<std::vector<SimTK::CableSpan>&>(m_cableSpans);

    /* std::cout << "build path for " << getName() << std::endl; */

    cableSpans.clear();
    Data::AttachmentPoint prevPoint = m_data.origin;
    for (const Data::Span& segment : m_data.segments) {

        const SimTK::Vec3 originStation = prevPoint.station;
        const SimTK::MobilizedBodyIndex originBody = findBody(prevPoint.body);
        const SimTK::Vec3 terminationStation = segment.termination.station;
        const SimTK::MobilizedBodyIndex terminationBody =
                findBody(segment.termination.body);

        SimTK::CableSpan cable(cableSubsystem, originBody, originStation,
                terminationBody, terminationStation);
        /* std::cout << "new span from " << prevPoint.point_G << " to " */
        /*           << segment.termination.point_G << std::endl; */
        for (const Data::Obstacle& obstacle : segment.obstacles) {
            cable.addObstacle(findBody(obstacle.body), obstacle.X_BS,
                    obstacle.obstacleSurface, obstacle.contactPointHint_S);
        }

        cable.setCurveSegmentAccuracy(1e-9);
        const Real accuracy = 0.1 / 180. * SimTK::Pi;
        cable.setSmoothnessTolerance(accuracy);

        cableSpans.push_back(cable);
        prevPoint = segment.termination;
    }
    /* std::cout << "Done building path for " << getName() << std::endl; */
}
...
void GeometryPath::generateDecorations(bool fixed,
        const ModelDisplayHints& hints, const SimTK::State& state,
        SimTK::Array_<SimTK::DecorativeGeometry>& appendToThis) const {
    if (m_useCableSpan) {
        /* throw std::runtime_error("generateDecorations: not yet implemented"); */
        // NOTE removed
        return;
    }
    ...
    for (int i = 1; i < pathPoints.getSize(); ++i) {
        ...
        if (pwp) {
            ...
            // frame. Transform the surface points into the ground reference
            // frame to draw the surface point as the wrapping portion of the
            // GeometryPath
            const Transform& X_BG =
 ...
 *
 */
const OpenSim::Array<AbstractPathPoint*>& GeometryPath::getCurrentPath(
...
void GeometryPath::getPointForceDirections(const SimTK::State& s,
    ...
    if (m_useCableSpan) {
        ...
        /*         "getPointForceDirections: not yet implemented"); */
        // NOTE removed
        return;
    }
    int i;
    ...
    for (i = 0; i < np - 1; i++) {
        ...
        if (startBody != endBody) {
            ...
            // engine.getPosition(s, start->getParentFrame(),
            // start->getLocation(), posStart);
            posStart = start->getLocationInGround(s);

            // engine.getPosition(s, end->getParentFrame(), end->getLocation(),
            // posEnd);
...
void GeometryPath::addInEquivalentForces(const SimTK::State& s,
    ...
    if (m_useCableSpan) {
        addCableSpanInEquivalentForces(s, m_cableSpans, tension, bodyForces);
        return;
    }
    ...
    for (int i = 0; i < np - 1; ++i) {
        ...
        if (bo != bf) {
            ...
            const MovingPathPoint* mppo = dynamic_cast<MovingPathPoint*>(start);

            // do the same for the end point of this segment of the path
            const MovingPathPoint* mppf = dynamic_cast<MovingPathPoint*>(end);

            // add in the tension point forces to body forces
            if (mppo) { // moving path point location is a function of the state
                // transform of the frame of the point to the base mobilized
                // body
                auto X_BF = mppo->getParentFrame().findTransformInBaseFrame();
                bo->applyForceToBodyPoint(
                        s, X_BF * mppo->getLocation(s), force, bodyForces);
            } else {
                // transform of the frame of the point to the base mobilized
                // body
                auto X_BF = start->getParentFrame().findTransformInBaseFrame();
                bo->applyForceToBodyPoint(
                        s, X_BF * start->getLocation(s), force, bodyForces);
            }
            ...
            if (mppf) { // moving path point location is a function of the state
                ...
                bf->applyForceToBodyPoint(
                        s, X_BF * mppf->getLocation(s), -force, bodyForces);
            } else {
                // transform of the frame of the point to the base mobilized
                // body
                auto X_BF = end->getParentFrame().findTransformInBaseFrame();
                bf->applyForceToBodyPoint(
                        s, X_BF * end->getLocation(s), -force, bodyForces);
            }
            ...
            if (mppo) {
                ...
                dPodq_G = bo->expressVectorInGroundFrame(
                        s, start->getdPointdQ(s));
                fo = ~dPodq_G * force;

                // get the mobilized body the coordinate is couple to.
                const SimTK::MobilizedBody& mpbod = matter.getMobilizedBody(
                        mppo->getXCoordinate().getBodyIndex());

                // apply the generalized (mobility) force to the coordinate's
                // body
                mpbod.applyOneMobilityForce(s,
                        mppo->getXCoordinate().getMobilizerQIndex(), fo,
                        mobilityForces);
            }

            if (mppf) {
                dPfdq_G =
                        bf->expressVectorInGroundFrame(s, end->getdPointdQ(s));
                ff = ~dPfdq_G * (-force);

                // get the mobilized body the coordinate is couple to.
                const SimTK::MobilizedBody& mpbod = matter.getMobilizedBody(
                        mppf->getXCoordinate().getBodyIndex());

                mpbod.applyOneMobilityForce(s,
                        mppf->getXCoordinate().getMobilizerQIndex(), ff,
                        mobilityForces);
            }
        }
    }
...
void GeometryPath::updateGeometry(const SimTK::State& s) const {
    if (m_useCableSpan) {
        for (const CableSpan& cable: m_cableSpans) {
            cable.calcLength(s);
        }
        return;
    }
...
double GeometryPath::getLength(const SimTK::State& s) const {
    if (m_useCableSpan) { return calcCableSpanLength(s, m_cableSpans); }
...
void GeometryPath::setLength(const SimTK::State& s, double length) const {
    if (m_useCableSpan) {
        /* throw std::runtime_error("setLength: not yet implemented"); */
        // NOTE removed
        return;
    }
...
double GeometryPath::getLengtheningSpeed(const SimTK::State& s) const {
    if (m_useCableSpan) {
        return calcCableSpanLengtheningSpeed(s, m_cableSpans);
    }
    computeLengtheningSpeed(s);
    return getCacheVariableValue(s, _speedCV);
}
void GeometryPath::setLengtheningSpeed(
        const SimTK::State& s, double speed) const {
...
AbstractPathPoint* GeometryPath::addPathPoint(
        const SimTK::State& s, int aIndex, const PhysicalFrame& frame) {
    PathPoint* newPoint = new PathPoint();
    newPoint->setParentFrame(frame);
    Vec3 newLocation(0.0);
    // Note: placeNewPathPoint() returns a location by reference.
    // It computes a new location according to the index where the new path
    // point will be inserted along the path(among the other path points).
...
AbstractPathPoint* GeometryPath::appendNewPathPoint(
        const std::string& proposedName, const PhysicalFrame& frame,
 ...
 */
void GeometryPath::placeNewPathPoint(const SimTK::State& s,
        SimTK::Vec3& aOffset, int aIndex, const PhysicalFrame& frame) {
    // The location of the point is determined by moving a 'distance' from
    // 'base' along a vector from 'start' to 'end.' 'base' is the existing path
    // point that is in or closest to the index aIndex. 'start' and 'end' are
    // existing path points--which ones depends on where the new point is being
    // added. 'distance' is 0.5 for points added to the middle of a path (so the
    // point appears halfway between the two adjacent points), and 0.2 for
    // points that are added to either end of the path. If there is only one
    // point in the path, the new point is put 0.01 units away in all three
    // dimensions.
...
bool GeometryPath::canDeletePathPoint(int aIndex) {
    ...
    if (numOtherFixedPoints >= 2) return true;
...
bool GeometryPath::deletePathPoint(const SimTK::State& s, int aIndex) {
    if (canDeletePathPoint(aIndex) == false) return false;
...
bool GeometryPath::replacePathPoint(const SimTK::State& s,
        AbstractPathPoint* aOldPathPoint, AbstractPathPoint* aNewPathPoint) {
...
void GeometryPath::extendPreScale(
    ...
    if (m_useCableSpan) {
        std::cout << "WARNING: extendPreScale was removed\n";
        return;
    }
...
void GeometryPath::extendPostScale(
        const SimTK::State& s, const ScaleSet& scaleSet) {
    if (m_useCableSpan) {
        std::cout << "WARNING: extendPostScale was removed\n";
        return;
    }
...
void GeometryPath::computePath(const SimTK::State& s) const {
    if (isCacheVariableValid(s, _currentPathCV)) {
        // even though the cache variable is valid, re-populate the pointers
        // cache
        //
        // this is because although it may be valid *in the state* the
        // model-level pointers cache may be dirty with lookups from other
        // states
        PopulatePathPointersCache(get_PathPointSet(), get_PathWrapSet(),
    ...
    for (int i = 0; i < get_PathPointSet().getSize(); i++) {
        if (get_PathPointSet()[i].isActive(s))
            _currentPathPtrsCache.append(
                    &get_PathPointSet()[i]); // <--- !!!!BAD
    }
    ...
    //
    // because the path is state-dependent (different states may wrap
    // differently), the path points (abstract) must be stored in a SimTK cache
    // variable.
    //
    // However, we can't store raw pointers in a cache variable because that may
    // cause aliasing issues in downstream code. E.g. the state may later be
    // used with a copied/moved-from version of the model, rather than the
    // original one, so the pointers may be stale.
    std::vector<PathElementLookup>& lookups =
            updCacheVariableValue(s, _currentPathCV);
    PopulatePathElementLookup(get_PathPointSet(), get_PathWrapSet(),
            _currentPathPtrsCache, *this, lookups);
...
void GeometryPath::computeLengtheningSpeed(const SimTK::State& s) const {
    if (isCacheVariableValid(s, _speedCV)) { return; }
...
void GeometryPath::applyWrapObjects(
        const SimTK::State& s, Array<AbstractPathPoint*>& path) const {
    int wrapSetSize = get_PathWrapSet().getSize();
    if (wrapSetSize < 1) return;
    ...
    for (int i = 0; i < wrapSetSize; i++) order[i] = i;
    ...
    for (int kk = 0; kk < maxIterations; kk++) {
        for (int i = 0; i < wrapSetSize; i++) {
            ...
            if (wo->get_active()) {
                ...
                const int wrapStart =
                        (ws.getStartPoint() < 1 ? 0 : ws.getStartPoint() - 1);
                const int wrapEnd =
                        (ws.getEndPoint() < 1 ? get_PathPointSet().getSize() - 1
                                              : ws.getEndPoint() - 1);
                ...
                for (; jfwd <= wrapEnd; jfwd++)
                    if (get_PathPointSet().get(jfwd).isActive(s)) break;
                if (jfwd > wrapEnd) // there are no active points in the path
                    return;
                const AbstractPathPoint* const smp =
                        &get_PathPointSet().get(jfwd);
                ...
                for (; jrev >= wrapStart; jrev--)
                    if (get_PathPointSet().get(jrev).isActive(s)) break;
                if (jrev < wrapStart) // there are no active points in the path
                    return;
                const AbstractPathPoint* const emp =
                        &get_PathPointSet().get(jrev);

                // 4. Now find the indices of smp and emp in _currentPath.
                int start = -1, end = -1;
                for (int j = 0; j < path.getSize(); j++) {
                    if (path.get(j) == smp) start = j;
                    if (path.get(j) == emp) end = j;
                }
                ...
                for (int pt1 = start; pt1 < end; pt1++) {
                    ...
                    if (path.get(pt1)->getWrapObject() == NULL ||
                        ...
                        result[i] = wo->wrapPathSegment(
                                s, *path.get(pt1), *path.get(pt2), ws, wr);
                ...
                if (best_wrap.wrap_pts.getSize() == 0) {
                    ...
                    ws.updWrapPoint2().setWrapLength(
                            s, best_wrap.wrap_path_length);
 ...
 */
double GeometryPath::calcPathLengthChange(const SimTK::State& s,
        const WrapObject& wo, const WrapResult& wr,
 ...
 */
double GeometryPath::calcLengthAfterPathComputation(const SimTK::State& s,
...
void GeometryPath::updateFromXMLNode(
    ...
    if (versionNumber < XMLDocument::getLatestVersion()) {
        if (versionNumber < 30516) {
            ...
            SimTK::Xml::element_iterator visObjectIter =
                    aNode.element_begin("VisibleObject");
            if (visObjectIter != aNode.element_end()) {
                SimTK::Xml::element_iterator oldPrefIter =
                        visObjectIter->element_begin("display_preference");
                // old display_preference was used only for hide/show other
                // options unsupported
                if (oldPrefIter != visObjectIter->element_end()) {
                    ...
                    if (oldPrefAsInt == 0) { // Hidden => Appearance/Visible
                        ...
                        appearanceElement.insertNodeAfter(
                                appearanceElement.element_end(),
                                visibleElement);
                    }
                }
            }
            // If default_color existed, copy it to Appearance/color
            SimTK::Xml::element_iterator defaultColorIter =
                    aNode.element_begin("default_color");
            if (defaultColorIter != aNode.element_end()) {
                // Move default_color to Appearance/color
                SimTK::Xml::Element colorElement("color");
                const SimTK::String& colorAsString =
                        defaultColorIter->getValue();
