...
const OpenSim::PathPointSet& OpenSim::PointBasedPath::getPathPointSet() const
{
    return get_PathPointSet();
}

OpenSim::PathPointSet& OpenSim::PointBasedPath::updPathPointSet()
{
    return upd_PathPointSet();
}

OpenSim::AbstractPathPoint* OpenSim::PointBasedPath::addPathPoint(
        const SimTK::State& s,
        int aIndex,
        const PhysicalFrame& frame)
{
    ...
    // was added. These indices are 1-based.    aIndex++;
...
OpenSim::AbstractPathPoint* OpenSim::PointBasedPath::appendNewPathPoint(
...
bool OpenSim::PointBasedPath::canDeletePathPoint(int aIndex)
{
...
bool OpenSim::PointBasedPath::deletePathPoint(const SimTK::State& s, int aIndex)
{
    if (!canDeletePathPoint(aIndex)) {
        return false;
    }
...
bool OpenSim::PointBasedPath::replacePathPoint(
        const SimTK::State& s,
...
const OpenSim::Array<OpenSim::AbstractPathPoint*>& OpenSim::PointBasedPath::getCurrentPath(
        const SimTK::State& s)  const
{
    computePath(s);   // compute checks if path needs to be recomputed
    return getCacheVariableValue< Array<AbstractPathPoint*> >(s, _currentPathCV);
}


const OpenSim::PathWrapSet& OpenSim::PointBasedPath::getWrapSet() const
{
    return get_PathWrapSet();
}

OpenSim::PathWrapSet& OpenSim::PointBasedPath::updWrapSet()
{
    return upd_PathWrapSet();
}

void OpenSim::PointBasedPath::addPathWrap(WrapObject& aWrapObject)
{
...
void OpenSim::PointBasedPath::moveUpPathWrap(const SimTK::State& s, int aIndex)
{
    if (aIndex > 0) {
...
void OpenSim::PointBasedPath::moveDownPathWrap(const SimTK::State& s, int aIndex)
{
...
void OpenSim::PointBasedPath::deletePathWrap(const SimTK::State& s, int aIndex)
{
    upd_PathWrapSet().remove(aIndex);
}
...
void OpenSim::PointBasedPath::setColor(const SimTK::State& s,
                                       const SimTK::Vec3& color) const
{
    setCacheVariableValue(s, _colorCV, color);
}


double OpenSim::PointBasedPath::getLength(const SimTK::State& s) const
{
    computePath(s);  // compute checks if path needs to be recomputed
    return getCacheVariableValue(s, _lengthCV);
}

void OpenSim::PointBasedPath::setLength(const SimTK::State& s, double length) const
{
    setCacheVariableValue(s, _lengthCV, length);
}


double OpenSim::PointBasedPath::getLengtheningSpeed(const SimTK::State& s) const
{
...
void OpenSim::PointBasedPath::addInEquivalentForces(
    const SimTK::State& s,
    double tension,
...
double OpenSim::PointBasedPath::computeMomentArm(
        const SimTK::State& s,
...
void OpenSim::PointBasedPath::extendPreScale(
        const SimTK::State& s,
...
void OpenSim::PointBasedPath::extendPostScale(
        const SimTK::State& s,
        const ScaleSet& scaleSet)
{
...
void OpenSim::PointBasedPath::updateGeometry(const SimTK::State& s) const
{
...
void OpenSim::PointBasedPath::extendFinalizeFromProperties()
{
...
void OpenSim::PointBasedPath::extendAddToSystem(SimTK::MultibodySystem& system) const
{
...
void OpenSim::PointBasedPath::extendInitStateFromProperties(SimTK::State& s) const
{
...
void OpenSim::PointBasedPath::generateDecorations(
        bool fixed,
...
{
    if (fixed) {
        return;  // there is no fixed geometry to generate here
    }
    ...
    SimTK::Vec3 lastPos = lastPoint->getLocationInGround(state);
...
void OpenSim::PointBasedPath::computePath(const SimTK::State& s) const
{
...
void OpenSim::PointBasedPath::computeLengtheningSpeed(const SimTK::State& s) const
{
...
void OpenSim::PointBasedPath::applyWrapObjects(
        const SimTK::State& s,
 ...
 */
double OpenSim::PointBasedPath::calcPathLengthChange(
        const SimTK::State& s,
        const WrapObject& wo,
        const WrapResult& wr,
 ...
 */
double OpenSim::PointBasedPath::calcLengthAfterPathComputation(
        const SimTK::State& s,
 ...
 */
void OpenSim::PointBasedPath::namePathPoints(int aStartingIndex)
{
    char indx[5];
 ...
 */
void OpenSim::PointBasedPath::placeNewPathPoint(
        const SimTK::State& s,
        SimTK::Vec3& aOffset,
        int aIndex,
        const PhysicalFrame& frame)
...
void OpenSim::PointBasedPath::updateFromXMLNode(
